...
public class UsersService {
	...
	public int updateMyPageInfo(UsersDTO dto) {
		// ID 기준으로 주민번호 등록 여부만 확인하는 메서드 새로 생성 (getUsersInfo()는 정보가 너무 많음)
		String ssnMasked = dao.getSsnMasked(dto.getId());

		boolean alreadyRegistered = ssnMasked != null 
				&& !ssnMasked.isBlank() && !"-".equals(ssnMasked);

		boolean hasNewSsn = dto.getSsn() != null && !dto.getSsn().isBlank();

		if (alreadyRegistered && hasNewSsn) {
			throw new IllegalStateException("주민등록번호는 최초 등록 후 변경할 수 없습니다.");
		}
